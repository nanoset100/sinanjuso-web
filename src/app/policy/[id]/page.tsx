...
export default function PolicyDetailPage({ params }: { params: { id: string } }) {
  const policy = KEY_POLICIES_8.find((p) => p.id === params.id);
  if (!policy) notFound();

  const idx = KEY_POLICIES_8.findIndex((p) => p.id === params.id);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      {/* 상단 배너 */}
      <div className="w-full py-12 px-4" style={{ backgroundColor: policy.badgeColor }}>
        <div className="max-w-3xl mx-auto text-center">
          ...
          <h1 className="text-3xl sm:text-5xl font-black text-[#1B3A6B] mb-4 break-keep">{policy.title}</h1>
          <p className="text-xl font-bold" style={{ color: policy.badgeTextColor }}>{policy.hook}</p>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 pt-12">
        ...
        <div className="bg-white rounded-3xl shadow-lg p-8 sm:p-12 mb-8">
          <h2 className="text-xl font-black text-[#1B3A6B] mb-6 flex items-center gap-2">
